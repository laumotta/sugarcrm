...
class cstm_quotesViewList extends ViewList {
...
if($ROLc=='GERENTE' || $ROLc=='DISTRIBUIDOR' || $ROLc=='ASESOR LIDER'  || $ROLc=='ASESOR'  ){
			if( $this->where != "") {

				if(empty($result2)){
                                $query_ =sprintf("and cstm_quotes.assigned_user_id  in ('%s')",$IDc);

                                }else {
                                $query_ =sprintf("and cstm_quotes.assigned_user_id  in (%s,'%s')",$result2,$IDc);
				...
				if(empty($result2)){
                                $query_ =sprintf("cstm_quotes.assigned_user_id  in ('%s')",$IDc);

                                }else {
                                $query_ =sprintf("cstm_quotes.assigned_user_id  in (%s,'%s')",$result2,$IDc);

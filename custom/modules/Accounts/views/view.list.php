	...
	function listViewProcess(){
		...
		if($ROLc=='GERENTE' || $ROLc=='DISTRIBUIDOR' || $ROLc=='ASESOR LIDER' ){
			if( $this->where != "") {
				...
				if(empty($result2)){
			         $query_ =sprintf("and accounts.assigned_user_id  in ('%s')",$IDc);
                                }else {
                                $query_ =sprintf("and accounts.assigned_user_id  in (%s,'%s')",$result2,$IDc);
                                }
				...
				if(empty($result2)){
                                $query_ =sprintf("accounts.assigned_user_id  in ('%s')",$IDc);
                                }else {
                                 $query_ =sprintf("accounts.assigned_user_id  in (%s,'%s')",$result2,$IDc);
                                }

...
class cstm_commissionsViewList extends ViewList {
	...
	function listViewProcess(){
		...
		if($ROLc=='GERENTE' || $ROLc=='DISTRIBUIDOR' || $ROLc=='ASESOR LIDER' ){
			if( $this->where != "") {
				...
				if(empty($result2)){
				$query_ =sprintf("and cstm_commissions.assigned_user_id  in ('%s')",$IDc);

				}else {
				$query_ =sprintf("and cstm_commissions.assigned_user_id  in (%s,'%s')",$result2,$IDc);
				...
				if(empty($result2)){
				$query_ =sprintf("cstm_commissions.assigned_user_id  in ('%s')",$IDc);
				}else{
				$query_ =sprintf("cstm_commissions.assigned_user_id  in (%s,'%s')",$result2,$IDc);
				}				
				$this->where .= $query_;
			}
		}

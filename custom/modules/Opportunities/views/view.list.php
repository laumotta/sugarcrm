	...
	function listViewProcess(){
		...
		if($ROLc=='GERENTE' || $ROLc=='DISTRIBUIDOR' || $ROLc=='ASESOR LIDER' || $ROLc=='ASESOR'  ){
			if( $this->where != "") {
				...
				if(empty($result2)){
				$query_ =sprintf("and opportunities.assigned_user_id  in ('%s')",$IDc);

				}else {
				$query_ =sprintf("and opportunities.assigned_user_id  in (%s,'%s')",$result2,$IDc);
				...
				if(empty($result2)){
				$query_ =sprintf("opportunities.assigned_user_id  in ('%s')",$IDc);
				}else{
				$query_ =sprintf("opportunities.assigned_user_id  in (%s,'%s')",$result2,$IDc);
				}				
				$this->where .= $query_;
			}
		}
